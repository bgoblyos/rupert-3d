...
-- Use Chazelle's algorithm instead if possible
isInside point polygon = fst $ foldl tester (True, firstPoint) wrapped
  where firstPoint = head polygon
        wrapped = tail polygon ++ [firstPoint]
        tester (result, a) b = if result
                             then (isRight a b point, b)
                             else (False, (0,0))
